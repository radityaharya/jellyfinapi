...
class TestHelper(object):
    ...
    @staticmethod
    def match_headers(expected_headers, received_headers, allow_extra=True):
        ...
        if type(expected_body) == dict:
            ...
            if check_count == True and (len(expected_body) != len(received_body)):
                ...
                for i, expected_element in enumerate(expected_body):
                    matches = [
                        j
                        for j, received_element in enumerate(received_body)
                        if TestHelper.match_body(
                            expected_element,
                            received_element,
                            check_values,
                            check_order,
                    ...
                    if check_order == True:
                        if i != 0 and all(
                            [all(y > x for y in previous_matches) for x in matches]
                        ):
                            return False
