...
class APIHelper(object):
    ...
    def get_schema_path(path, file_name):
        ...
        path = (
            path.replace("\\models", "\\schemas")
            .replace("/models", "/schemas")
            .replace(".py", ".json")
            .replace(file_name, pascal_name)
        )
        return path
    ...
    def append_url_with_template_parameters(url, parameters):
        ...
        for key in parameters:
            ...
            if value is None:
                ...
                replace_value = "/".join(
                    (quote(str(x), safe="") if encode else str(x)) for x in value
                )
            else:
    ...
    def append_url_with_query_parameters(
        ...
        if parameters is None:
            return url
        parameters = APIHelper.process_complex_types_parameters(
            parameters, array_serialization
        )
    ...
    def process_complex_types_parameters(query_parameters, array_serialization):
        ...
        for key, value in query_parameters.items():
            processed_params.extend(
                APIHelper.form_encode(
                    value, key, array_serialization=array_serialization, is_query=True
                )
            )
    ...
    @staticmethod
    def form_encode(obj, instance_name, array_serialization="indexed", is_query=False):
        ...
        if obj is None:
            ...
            for element in APIHelper.serialize_array(
                instance_name, obj, array_serialization, is_query
            ):
                retval += APIHelper.form_encode(
                    element[1], element[0], array_serialization, is_query
                )
